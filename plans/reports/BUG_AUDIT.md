...
- **Status:** 🟡 PARTIAL — scan snapshots now encode and atomically write on a coalescing serial background queue, with a lifecycle flush on app background; unbounded scan/tombstone retention remains a scale backlog pending deletion acknowledgement from CloudKit (2026-07-15)
...
- **Detail:** The original path encoded `scans + tombstones` and called Data.write synchronously from MainActor mutations. `JSONSnapshotWriter` now snapshots immediately, preserves write ordering, coalesces rapid mutations to the newest pending value, and performs encoding plus atomic I/O off-main. Nothing yet caps or safely prunes the array: deleteScan/deleteAllScans convert records to tombstones that must remain until CloudKit deletion acknowledgement exists, or an offline device could resurrect deleted scans.
...
- **Status:** ✅ FIXED — photo reads use `loadPhotoAsync`; drink and journal snapshots now encode and atomically write on coalescing serial background queues, with pending writes flushed when the app backgrounds (2026-07-15)
...
- **Detail:** The original saveDrinks/saveEntries paths encoded full arrays and wrote them inline from MainActor callers. Both now hand immutable value snapshots to `JSONSnapshotWriter`, which serializes/coalesces work off-main while preserving atomic latest-write-wins ordering. Persistence round-trip tests explicitly flush the queue, and RootView flushes all three stores on scene background. Photo rendering uses the async cache-backed load path.
