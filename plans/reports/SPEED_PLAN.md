...
| 11 | `ScanStore.swift:109` (+ `DrinkStore.saveDrinks:115`, `JournalStore.saveEntries:105`) | Shared persistence pattern: snapshot arrays on main, encode+atomic-write on one serial background `DispatchQueue` per store (mirror `ScanLog`), coalesce with a ~500ms debounce. | 5–200ms main stall off verdict presentation, Save-for-Later, and every mutation |
