...
Multiple Codex sessions work on this repo in parallel. The repo is the coordination
...
3. Shared findings live in `plans/reports/` — read `BUG_AUDIT.md` (57 verified
   ...
   E2E/tactical track.
4. Use a lowercase tool prefix for task branches: `codex/<task>`,
   `claude/<task>`, or `cli/<task>`. Pushes to any of these prefixes run the
   compile/sign and simulator gates (doc/script/config-only changes excluded).
   Only `main` (or a manual dispatch) uploads to TestFlight, so never put an
   intermediate commit on `main`.

## End-to-End Delivery Contract (EVERY coding session)
The agent that implements a change owns delivery. Do not stop after editing or
testing and ask the user to run Git commands.

1. Work on a dedicated prefixed task branch, claim the files in Active Tracks,
   and commit only files owned by that task. Never sweep unrelated untracked or
   user-authored files into a commit.
2. Run the focused local tests/build appropriate to the change, then fetch and
   rebase onto the latest `origin/main` before the final branch push.
3. Push the task branch and wait for its GitHub Actions compile/sign and E2E
   checks. Fix failures on the same branch. Docs/tooling-only changes may have
   no app build by design.
4. After required checks pass, fast-forward the verified commit to `main`. If
   `main` advanced, rebase, rerun affected checks, and retry; never force-push
   `main`.
5. For app-shipping changes, watch the `main` TestFlight run through
   `UPLOAD_OK`, which means the build was processed and assigned to internal
   testers. Report the run URL and TestFlight build number to the user.
6. Release the Active Tracks reservation in the same merge. Leave the local
   checkout on an up-to-date `main` unless another active task requires its
   branch.

This delivery contract applies to Codex app, Codex CLI, Claude, and manual CLI
sessions. A user request to pause, avoid committing, avoid pushing, or only
review/analyze overrides automatic delivery.

**Active Tracks** (update when claiming/releasing):
| Track | Branch | Reserved files |
|---|---|---|
| Design modernization (completed/merged) | `Codex/review-project-status-5ihff` | Released. The product changes are in `main`; no visual files remain reserved by this track. |
| Simulator E2E tap-and-fix (completed/merged) | `Codex/ios-simulator-e2e-testing-t5mxgm` | Released. Its handoff items were absorbed by the functional E2E finish; no files remain reserved. |
| Functional E2E finish (completed 2026-07-15) | `Codex/codex-e2e-finish` | Released after simulator verification; no files remain reserved by this track. |
| Physical-device hardening (completed 2026-07-15) | `Codex/codex-device-hardening` | Released after simulator, signed-device-build, and 97-test verification. Real camera/Foundation Models output still needs an unlocked physical device; no files remain reserved. |
| Final polish audit (completed 2026-07-15) | `Codex/codex-final-polish-audit` | Released after compact/accessibility E2E, real-photo OCR + persistence validation, 98 passing tests, signed Release build, and reviewed 30 fps walkthrough. Live camera/DataScanner + Foundation Models output still require an unlocked physical iPhone; no files remain reserved. |
| Persistence finish (completed 2026-07-15) | `Codex/codex-persistence-finish` | Released after 48 focused persistence tests, 98 full tests, local simulator/device Release builds, signed branch build 90, and published E2E run 41. No files remain reserved. |
| Physical validation + recommendation stress tests (completed 2026-07-15) | `Codex/codex-physical-validation` | Released after 112 tests, signed iPhone build plus live DataScanner/Foundation Models smoke, 17-photo corpus validation, signed CI archive, and E2E run 42. No files remain reserved. |
...
| Founder video feedback follow-up (active 2026-07-24) | `codex/video-feedback-fixes` | `AGENTS.md`; `.github/workflows/e2e-simulator.yml`; `plans/reports/BEER_DRINKER_ARCHETYPES.md`; `plans/reports/FOUNDER_FEEDBACK_2026-07-24.md`; `SipCheck/Views/OnboardingView.swift`; `SipCheck/Services/TastePreferences.swift`; `SipCheck/Views/Components/CameraView.swift`; `SipCheck/Views/Tabs/CheckTabView.swift`; `SipCheck/Views/Tabs/SettingsTabView.swift`; `SipCheck/Views/Components/VerdictCardView.swift`; `SipCheck/Resources/Assets.xcassets/BeerBrands/`; `SipCheckTests/RecommendationArchetypeEvalTests.swift`; `SipCheckTests/PipelineIntegrationTests.swift`; `SipCheckUITests/SipCheckUITests.swift`; `scripts/capture_product_baseline.py`; `SipCheck.xcodeproj/project.pbxproj`. |
...
- User drives coding sessions from Codex, Claude, and CLI clients on iPhone or
  MacBook Pro. Data syncs across devices via CloudKit (last-write-wins), so
  taste history is shared.
...
- Start a session: edit `.drive/request.json` on a supported agent branch and push
...
Codex mcp add -s user XcodeBuildMCP npx xcodebuildmcp@latest
...
## Current Status
- Build: ✅ Passing
- Core flows: ✅ Implemented and simulator-verified, including real-photo OCR and photo persistence
- Tests: ✅ 98 unit, integration, and UI tests passing
- Release assets: ✅ Opaque 1024px app icon and dark-only launch transition configured
- Pending: live camera/DataScanner and Foundation Models wording validation on an unlocked Apple-Intelligence-capable physical iPhone
