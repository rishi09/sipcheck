...
| Live long-tail beer discovery (active) | `codex/long-tail-search` | `AGENTS.md`, `plans/reports/FOUNDER_TASTE.md`, `docs/privacy/index.html`, `.github/workflows/testflight.yml`, `.github/workflows/cloudkit-schema.yml`, `scripts/assert_cloudkit_scan_schema.py`, `SipCheck.xcodeproj/project.pbxproj`, `SipCheck/SipCheckApp.swift`, typed-search/resolver/network service files, `Models/Scan.swift`, `Services/CloudKitSyncService.swift`, remote-fact provenance views (`VerdictCardView`, `AddBeerView`, `ProfileTabView`, `JournalTabView`, `JournalEntryDetailView`, `FollowUpView`), and focused long-tail unit/UI tests. |
